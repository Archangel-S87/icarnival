				...
				{foreach $services_categories as $ac2}
					{if $ac2->visible}
						<li class="product" onClick="window.location='/services/{$ac2->url}'">
							<div class="image">
							{if $ac2->image}
								<img loading="lazy" alt="{$ac2->menu|escape}" title="{$ac2->menu|escape}" src="{$config->services_categories_images_dir}{$ac2->image}" />
							{else}
								<svg class="nophoto"><use xlink:href='#folder' /></svg>
							{/if}
							</div>
...
{if !empty($settings->hideblog)}
	<div class="mainproduct blue noradius">Новости</div>
