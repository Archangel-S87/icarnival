	...
	<div class="product_info product_item">
		...
		<div {if !empty($smarty.cookies.view) && $smarty.cookies.view == 'table' && $mod == 'ProductsView'}id="prod_right"{/if}>
			...
			<form class="variants" action="/cart">
				{if $product->vproperties}
					...
					<div>
						...
						{if $cntname2 == 0 || $cntname1 == 0}
							{if !empty($smarty.cookies.view) && $smarty.cookies.view == 'table' && $mod == 'ProductsView'}<div class="filler-sm"></div>{/if}
						{/if}
					</div>
				...
				{/if}
				<input {if $settings->trigger_id}onmousedown="try { rrApi.addToBasket({$product->variant->id}) } catch(e) {}"{/if} {if !empty($product->ref_url)}onClick="window.open('{$product->ref_url|escape}', '_blank');return false;"{/if} type="submit" class="buttonred" value="{$settings->to_cart_name|escape}" data-result-text="добавлено"/>
			</form>
			{else}
