...
{foreach from=$settings->position_carousel key=$carousel item=$i}
	{if !empty($settings->mainhits) && $carousel == 'mainhits'}
		{* hits *}
			...
			<div class="mainproduct block-header margin_top hits_carousel">
				...
				<div class="block-header__arrows-list">
					<button class="block-header__arrow arrow_left" type="button">
						<svg>
							<use xlink:href='#arrow-rounded-left-7x11'/>
						</svg>
					</button>
					<button class="block-header__arrow arrow_right" type="button">
						<svg>
							<use xlink:href='#arrow-rounded-right-7x11'/>
						</svg>
					</button>
				</div>
			</div>
	...
	{if !empty($settings->mainnew) && $carousel == 'mainnew'}
		{* new *}
		{* available sort (or remove): position, name, date, rating, rand *}
		...
		{* new end *}
	{/if}
	{if !empty($settings->mainsale) && $carousel == 'mainsale'}
		{* discounted *}
		{* available sort (or remove): position, name, date, rating, rand *}
