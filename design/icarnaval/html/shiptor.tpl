...
<div id="shiptor_widget" class="_shiptor_widget" 
	{if !empty($total_weight)}data-weight="{$total_weight}"{/if} 
	data-mode="inline" 
	...
	{if !empty($delivery->option5)}data-yk= "{$delivery->option5}" {/if}
	{* наценка: data-markup= "10%" или data-markup= "50" *}
	{* увеличение сроков: data-addDays= "5" *}
	{* фиксированная цена в руб.: data-fixedDeliveryPrice= "125" *}
	></div>
<script>
