<!-- incl. 1click -->
<!--noindex-->
<a href="javascript:;" class="various oneclick">купить в 1 клик</a>
<div style="display: none;">
    <div id="oneclick" class="window">
        <div class="title font">{$product->name|escape|rtrim}</div>
        <ul>
            <li><span>* Ваше имя:</span>
                <input class="onename"
                       value="{if isset($comment_name)}{$comment_name|escape}{/if}"
                       type="text"/></li>
            <li><span>* Email:</span>
                <input name="email"
                       class="onemail"
                       value="{if isset($user->email)}{$user->email|escape}{/if}"
                       type="email"/>
            </li>
            <li><span>* Телефон:</span>
                <input name="onephone"
                       class="onephone2"
                       value="{if isset($phone)}{$phone}{elseif isset($user->phone)}{$user->phone}{/if}"
                       type="text"/></li>
        </ul>
        <div id="oneclick_result"></div>
        {include file='conf.tpl'}
        <button type="submit"
                {if $settings->counters || $settings->analytics}onclick="{if $settings->counters}ym({$settings->counters},'reachGoal','cart'); {/if}{if $settings->analytics}ga ('send', 'event', 'cart', 'order_button');{/if} return true;"{/if}
                name="enter" value="1" class="oneclickbuy buttonred hideablebutton">Купить!
        </button>
    </div>
</div>
{literal}
