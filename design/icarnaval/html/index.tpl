...
<head>
	...
	<link rel="stylesheet" type="text/css" href="/js/jquery/fancybox-master/dist/jquery.fancybox.min.css"/>
...
<body {if $module}class="{$module|lower}"{/if}>
			...
			<div id="top_phone">
				<div class="divider {if $settings->phone && $settings->tel}twophone{else}onephone{/if}">
					...
					{if $settings->phone && $settings->tel}
						<div class="second-level font first_phone pointer" onClick="window.location='tel:{$settings->phone|escape|regex_replace:'/[() -]/' :''}'">{$settings->phone|escape}</div>
						<div class="second-level font pointer" onClick="window.location='tel:{$settings->tel|escape|regex_replace:'/[() -]/' :''}'">{$settings->tel|escape}</div>
					{elseif $settings->phone}
						<div class="first-level font">Свяжитесь с нами:</div>
						<div class="second-level font pointer" onClick="window.location='tel:{$settings->phone|escape|regex_replace:'/[() -]/' :''}'">{$settings->phone|escape}</div>
					{elseif $settings->tel}
						<div class="first-level font">Свяжитесь с нами:</div>
						<div class="second-level font pointer" onClick="window.location='tel:{$settings->tel|escape|regex_replace:'/[() -]/' :''}'">{$settings->tel|escape}</div>
					{/if}
			...
			<div class="inner">
				<ul class="super-menu">
					...
					<li class="catalog_menu">
						...
						{if !empty($settings->show_nav_cat)}
										...
										{foreach $categories as $c}
											...
											<li {if !empty($c->subcategories)}class="has_subcat"{/if}>
												<a title="{$c->name|escape}" href="catalog/{$c->url}">
													<span class="cat_name">{if !empty($c->menu)}{$c->menu|escape}{else}{$c->name|escape}{/if}</span>
							...
							{elseif !empty($settings->purpose) && !empty(services_categories)}
								{function name=services_tree_top level=1}
									<ul class="dropdown-menu">
										{foreach $services_categories as $c}
											{if $c->visible}
											<li {if !empty($c->subcategories)}class="has_subcat"{/if}>
												<a title="{$c->name|escape}" href="services/{$c->url}">
													<span class="cat_name">{$c->menu|escape}</span>
													{if !empty($c->subcategories)}<span class="subcat_icon"><svg><use xlink:href='#arrow' /></svg></span>{/if}
												</a>
												{if !empty($c->subcategories)}
													{services_tree_top services_categories=$c->subcategories level=$level+1}
												{/if}
											</li>
											{/if}
										{/foreach}
								...
								{/function}
								{services_tree_top services_categories=$services_categories}
							{/if}
						{/if}
					</li>
				...
				<div class="cart pointer" onClick="window.location='/cart'">
		...
		<div id="footer">
			...
			<div class="connect">
				<div class="contact-methods">
					{if $settings->phone || $settings->tel}
						<div class="contact-method"><span class="phone-20x20 sprite contact-img"></span><a rel="nofollow" href="tel:{if $settings->phone}{$settings->phone|escape|regex_replace:'/[() -]/' :''}{else}{$settings->tel|escape}{/if}">{if $settings->phone}{$settings->phone|escape}{else}{$settings->tel|escape|regex_replace:'/[() -]/' :''}{/if}</a></div>
					{/if}
			...
			<div class="custom-text">
				<div role="note">
					...
					{elseif $module == 'ProductView'}
						<p>Теги: {$product->name|escape}{if !empty($brand)}, {$brand->name|escape}{/if}
						{if !empty($category)}{foreach from=$category->path item=cat}, {$cat->name|escape}{/foreach}{/if}</p>
						{else}<p>{$meta_title|escape}</p>
					{/if}
				</div>
				{if $settings->disclaimer}
					<!--noindex--><div class="disclaimer">{$settings->disclaimer|escape}</div><!--/noindex-->
				{/if}
			</div>
		</div>
	...
	{/if}
	<script defer src="js/jquery/fancybox-master/dist/jquery.fancybox.js"></script>

	<script defer src="design/{$settings->theme|escape}/js/plugins_min.js?v={filemtime("design/{$settings->theme|escape}/js/plugins_min.js")}"></script>

	{if $module == 'ProductView'}
		<script defer src="design/{$settings->theme|escape}/js/product.js"></script>
	...
	{if in_array($module, array('ProductsView', 'ArticlesView', 'BlogView', 'SurveysView'))}
		...
		<script defer src="design/{$settings->theme|escape}/js/ajax_pagination_min.js"></script>
		{* ajax pagination end *}
