                ...
                <li {if $delivery->price2 > 0}data-price="{$delivery->price|convert}"
					...
					(<span class="del_price" id="not-null-delivery-price-{$delivery->id}">
                        ...
                        {if in_array($delivery->id, array(3,4,114,121)) || $delivery->widget == 1}
                            <a class="hideBtn show_map" {if $delivery->id == 3}data-role="shiptor_widget_show"{/if}
                               href="javascript://">выбрать на карте</a>
                        {elseif $delivery->description}
                            <a class="hideBtn" href="javascript://" onclick="hideShow(this);return false;">подробнее</a>
                        {/if}
                        <div class="description"
                             {if $delivery->id != 3 && $delivery->id != 4 && $delivery->id != 114 && $delivery->id != 121 && $delivery->widget != 1}id="hideCont"{/if}>
...
<style>
    .del_pay_prod_tab #deliveries .description, .del_pay_prod_tab #payments .description {
        width: 100%;
        margin-top: 5px;
    }

    .del_pay_prod_tab ul#deliveries, .del_pay_prod_tab ul#payments {
        border: 0;
        padding: 0;
        margin-top: 10px;
    }

    .del_pay_prod_tab .deliveryinfo_wrapper, .del_pay_prod_tab .deliveryinfo {
        display: none;
    }
