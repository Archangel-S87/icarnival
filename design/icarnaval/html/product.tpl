...
<div class="product">
		...
		<div class="imagebig" id="lenss">
			<div class="big_middle">
				{if $product->image}
					...
					<a href="{$product->image->filename|resize:960:1440:w}" class="cloud-zoom" id="zoom" title="{$product->name|escape}" data-rel="{if !empty($product->image->color)}{$product->image->color}{else}gallery{/if}" data-fancybox="images" data-width="1024">
					{/if}
						<div onClick="$('.imagesmall a:visible:first').click();" class="image">
							<img src="{$product->image->filename|resize:480:720}" alt="{$product->name|escape}" title="{$product->name|escape}" class="imglenss" />
						</div>
		...
		{if $product->images|count>1}
			<div class="images">
				{foreach $product->images as $i=>$image}
					<div class="{if !$image->color}imgvisible{/if} cloud-zoom-gallery imagesmall" data-imcolor="{$image->color}" data-sm="{$image->filename|resize:480:720}">
						<a href="{$image->filename|resize:960:1440:w}" title="{$product->name|escape}" data-rel="{if !empty($image->color)}{$image->color}{else}gallery{/if}" data-fancybox="images" data-width="1024" class="zoom-loop">
	...
	<div class="container">
		...
		<ul class="tabs">
			...
			{if !empty($settings->del_pay)}<li><a href="#tab6" title="Способы оплаты и доставки">Способы оплаты и доставки</a></li>{/if}
		</ul>
...
{if !empty($notinstock)}
	{* Похожие товары *}
	{get_products var=related_products category_id=$category->id sort=rand limit=10}
...
{if !empty($category_products)}
	<div class="mainproduct block-header categories-products">
		<div class="block-header__title">Из этой же категории</div>
		<div class="block-header__divider"></div>
	...
	<div id="categories_content" class="tiny_products hoverable owl-carousel">
	...
	<script>
		$(window).load(function(){
			$('#categories_content').slick(carousel_options);
			$('.categories-products .arrow_left').on('click',function(){ $('#categories_content').slick('slickPrev') });
			$('.categories-products .arrow_right').on('click',function(){ $('#categories_content').slick('slickNext') });
		});
	</script>
{/if}
...
{if !empty($brands_products)}
	<div class="mainproduct block-header brands-products">
		<div class="block-header__title">Товары того же бренда</div>
		<div class="block-header__divider"></div>
		{if $brands_products|count > 4}
			<div class="block-header__arrows-list">
				<button class="block-header__arrow arrow_left" type="button">
					<svg><use xlink:href='#arrow-rounded-left-7x11'/></svg>
				</button>
	...
	<div id="brand_content" class="tiny_products hoverable owl-carousel">
	...
	<script>
		$(window).load(function(){
			$('#brand_content').slick(carousel_options);
			$('.brands-products .arrow_left').on('click',function(){ $('#brand_content').slick('slickPrev') });
			$('.brands-products .arrow_right').on('click',function(){ $('#brand_content').slick('slickNext') });
		});
	</script>
{/if}

{if !empty($related_products) || !empty($brands_products) || !empty($category_products)}
