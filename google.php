...
// Товары
$fivecms->db->query("SELECT v.price, v.id as variant_id, p.name as product_name, v.name as variant_name, v.position as variant_position, v.sku AS variant_sku, v.currency_id, p.id as product_id, p.url, p.annotation, pc.category_id, b.name AS brand, i.filename as image
...
while($p = $fivecms->db->result())
{
	...
	if(!empty($p->image)){
		...
		<item>
			...
			";
			if($p->image)
				print "<g:image_link>".$fivecms->design->resize_modifier($p->image, 1024, 768, 'w')."</g:image_link>
			";
			if($p->brand)
