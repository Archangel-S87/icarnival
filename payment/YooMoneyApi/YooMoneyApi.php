...
class YooMoneyApi extends Fivecms
{
            ...
            if (isset($settings['yookassa_api_send_check']) && $settings['yookassa_api_send_check']) {
                ...
                foreach ($purchases as $key => $purchase) {
                    ...
                    if($key == endKey($purchases)) {
						$purchase_price = $total_price/$purchase->amount;
					} else {
						$purchase_price = round(((float)$purchase->price * $discount), 2);
						$total_price -= $purchase_price*$purchase->amount;
					}
                    
                    $purchase_name = mb_substr(htmlentities($purchase->product_name . ($purchase->variant_name ? ' ' . $purchase->variant_name : '')), 0, 512);
                        
                    $builder->addReceiptItem($purchase_name, $purchase_price, $purchase->amount, $id_tax,
    ...
    private function getForm($button_text, $settings, $amount)
    {
        ...
        <form method="POST">
            ...
            if ($onKassaSide) {
            	if(!empty($settings['yoomoney_show_installments_button'])){
    ...
    public function getApiClient($shopId, $shopPassword, $logger)
    {
        ...
        $userAgent->setCms('Fivecms', $this->config->version);
        $userAgent->setModule('yoomoney-cms-fivecms', YOOMONEY_MODULE_VERSION);
