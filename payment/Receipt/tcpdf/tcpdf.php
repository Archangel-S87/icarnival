		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (isset($dom[$key]['value']) AND ($dom[$key]['value'][0] == 'h') AND (intval($dom[$key]['value'][1]) > 0) AND (intval($dom[$key]['value'][1]) < 7)) {
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if (($dom[$key]['value'] == 'tr') AND isset($dom[$key]['thead']) AND $dom[$key]['thead']) {
				...
				while ( ($key < $maxel) AND (!(
					(isset($dom[$key]['tag']) && $dom[$key]['tag'] AND $dom[$key]['opening'] AND ($dom[$key]['value'] == 'tr') AND (!isset($dom[$key]['thead']) OR !$dom[$key]['thead']))
					OR (isset($dom[$key]['tag']) && $dom[$key]['tag'] AND (!$dom[$key]['opening']) AND ($dom[$key]['value'] == 'table'))) )) {
					++$key;
				}
			}
			if (isset($dom[$key]['tag']) && $dom[$key]['tag'] OR ($key == 0)) {
			...
			// align lines
			if ($this->newline && isset($dom[$key]['value']) AND (strlen($dom[$key]['value']) > 0) AND ($dom[$key]['value'] != 'td') AND ($dom[$key]['value'] != 'th')) {
				$newline = true;
				$fontaligned = false;
			...
			if (isset($dom[$key]['tag']) && $dom[$key]['tag']) {
				if ($dom[$key]['opening']) {
			...
			} elseif (isset($dom[$key]['value']) && strlen($dom[$key]['value']) > 0) {
				// print list-item
		...
		switch($tag['value']) {
			...
			case 'table': {
				...
				foreach ($dom[($dom[$key]['parent'])]['trids'] as $j => $trkey) {
					...
					if (isset($prevtrkey) AND (isset($dom[$trkey]['startpage']) && $dom[$trkey]['startpage'] > $dom[$prevtrkey]['endpage'])) {
