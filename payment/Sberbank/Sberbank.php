...
class Sberbank extends Fivecms
{
    ...
    public function checkout_form($order_id, $button_text = null)
    {
		...
		if(isset($_SESSION['order'][$this->order->id]))
        ...
        if (!isset($order_status['actionCode'])) {
            ...
            // Заказ был создан, но не не был оплачен, можно продолжить оплату
            ...
            if (!isset($sber_session['formUrl']) OR $order_status['actionCode'] != '-100') {
                ...
                $button = '<p>Заказ не был оплачен.</p>' .
    ...
    private function normalize($purchases)
    {
         ...
         * для каждого товара
         */
        if (!empty($this->payment_method->sbr_taxProduct)) {
            $products_ids = [];
            foreach ($purchases as $item) {
