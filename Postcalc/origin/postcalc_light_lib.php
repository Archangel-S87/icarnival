 ...
 *
 */
function postcalc_request($From, $To, $Weight, $Valuation = 0, $Country = 'RU', $Box = '', $arrParcels = array(), $arrServices = array())
{
    ...
    $QueryString = "key=$config_key";
    $QueryString .= "&f=$From&t=$To&w=$Weight&v=$Valuation&c=$Country";

    $QueryString .= "&o=php&sw=PostcalcLight_2.7&cs=$config_cs&ib=$config_ib&co=$config_co";
    if ($config_d != 'now') $QueryString .= "&d=$config_d"; // Дата, по умолчанию - сегодня
    if ($config_r != 0.01) $QueryString .= "&r=$config_r"; // Округление, по умолчанию - до 2 знаков
    if ($config_pr > 0) $QueryString .= "&pr=$config_pr";  // Сбор за обработку всего заказа, по умолчанию 0
    if ($config_pk > 0) $QueryString .= "&pk=$config_pk";  // Сбор за упаковку одного отправления, по умолчанию 0
    if ($config_pa > 0) $QueryString .= "&pa=$config_pa";  // Делимое отправление или нет
    ...
    if ($arrParcels) {
        $QueryString .= "&p=" . implode(',', $arrParcels);
    } else if ($config_p != '') {
        // Устанавливаем из конфига
        $QueryString .= "&p=$config_p";
    }

    // Список доп.опций и услуг через запятую
    if ($arrServices) {
        $QueryString .= "&sv=" . implode(',', $arrServices);
    } else if ($config_sv != '') {
        // Устанавливаем из конфига
        $QueryString .= "&sv=$config_sv";
    }
...
{
    $UserAgent = 'PostcalcLight_2.7 ' . phpversion();
