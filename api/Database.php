...
class Database extends Fivecms
{
	private $mysqli;
	private $res;
	...
	public function connect($force_reconnect = null) {
		...
		else
		{
			...
			if ($this->config->debug) {
                ...
                $this->mysqli->query('SET profiling_history_size = 200');
            }
		}
	...
	public function dumpsetttings($filename)
	{
		$h = fopen($filename, 'w');
		$q = $this->placehold("SHOW FULL TABLES LIKE '__%';");		
		$result = $this->mysqli->query($q);
	...
	public function dumptexts($filename)
	{
		$h = fopen($filename, 'w');
		$q = $this->placehold("SHOW FULL TABLES LIKE '__%';");		
		$result = $this->mysqli->query($q);
		$this->dump_table($this->config->db_prefix."blog", $h);
		$this->dump_table($this->config->db_prefix."blog_categories", $h);
		$this->dump_table($this->config->db_prefix."images_post", $h);
		$this->dump_table($this->config->db_prefix."tags", $h);
		$this->dump_table($this->config->db_prefix."articles", $h);
		$this->dump_table($this->config->db_prefix."images_article", $h);
		$this->dump_table($this->config->db_prefix."articles_categories", $h);
		$this->dump_table($this->config->db_prefix."services_categories", $h);
		$this->dump_table($this->config->db_prefix."images_service", $h);
		$this->dump_table($this->config->db_prefix."surveys", $h);
		$this->dump_table($this->config->db_prefix."surveys_categories", $h);
		$this->dump_table($this->config->db_prefix."surveys_fields", $h);
		$this->dump_table($this->config->db_prefix."surveys_results", $h);
	    fclose($h);
	}
	
	public function dumpusers($filename)
	{
		$h = fopen($filename, 'w');
		$q = $this->placehold("SHOW FULL TABLES LIKE '__%';");		
		$result = $this->mysqli->query($q);
		$this->dump_table($this->config->db_prefix."mail", $h);
		$this->dump_table($this->config->db_prefix."maillist", $h);
		$this->dump_table($this->config->db_prefix."groups", $h);
		$this->dump_table($this->config->db_prefix."users", $h);
	    fclose($h);
	}
	
	public function dumpprices($filename)
	{
		$h = fopen($filename, 'w');
		$q = $this->placehold("SHOW FULL TABLES LIKE '__%';");		
		$result = $this->mysqli->query($q);
		$this->dump_table($this->config->db_prefix."variants", $h);
	    fclose($h);
	}
	
	public function dumporders($filename)
	{
		$h = fopen($filename, 'w');
		$q = $this->placehold("SHOW FULL TABLES LIKE '__%';");		
		$result = $this->mysqli->query($q);
		$this->dump_table($this->config->db_prefix."orders", $h);
		$this->dump_table($this->config->db_prefix."purchases", $h);
		$this->dump_table($this->config->db_prefix."labels", $h);
		$this->dump_table($this->config->db_prefix."orders_labels", $h);
	    fclose($h);
	}
	
	public function dumpproducts($filename)
	{
		$h = fopen($filename, 'w');
		$q = $this->placehold("SHOW FULL TABLES LIKE '__%';");		
		$result = $this->mysqli->query($q);
		$this->dump_table($this->config->db_prefix."brands", $h);
		$this->dump_table($this->config->db_prefix."categories", $h);
		$this->dump_table($this->config->db_prefix."categories_features", $h);
		$this->dump_table($this->config->db_prefix."features", $h);
		$this->dump_table($this->config->db_prefix."files", $h);
		$this->dump_table($this->config->db_prefix."images", $h);
		$this->dump_table($this->config->db_prefix."options", $h);
		$this->dump_table($this->config->db_prefix."products_categories", $h);
		$this->dump_table($this->config->db_prefix."related_products", $h);
		$this->dump_table($this->config->db_prefix."products", $h);
		$this->dump_table($this->config->db_prefix."variants", $h);
	    fclose($h);
	}
		...
		if($result)
		{
			...
			if($num_rows > 0)
			{
				...
				while( $row = $result->fetch_row())
				{
					...
					if($index < $num_rows-1 && $index != 0 && $index % 1000 == 0){
						fwrite($h, ";");
						fwrite($h,  "\nINSERT INTO `$table` (`$fields`) VALUES");
					}	
					elseif($index < $num_rows-1)
						fwrite($h,  ",");	
					else
						fwrite($h, ";");
