...
class Config
{
	...
	// для удобного доступа к ним. Например: $fivecms->config->db_user
	...
	{		
		...
		// Подпапка в которую установлена CMS относительно корня веб-сервера
		...
		$max_upload_filesize = min($max_upload, $max_post, $memory_limit);
		if(!empty($max_upload_filesize)){
			if($max_upload_filesize > 1024*1024)
				$this->vars['max_upload_filesize'] = $max_upload_filesize;
			else 
				$this->vars['max_upload_filesize'] = $max_upload_filesize*1024*1024;
		}
		// Если соль не определена, то будем генерировать ее
