...
class Orders extends Fivecms
{
	...
	function get_orders($filter = array())
	{
		...
		$query = $this->db->placehold("SELECT o.id, o.delivery_id, o.delivery_price, o.separate_delivery,
										...
										o.url, o.total_price, o.note, o.track, o.bonused, o.shipping_date, o.referer, o.source, o.utm, o.yclid
									FROM __orders AS o 
	...
	public function update_order($id, $order)
	{
		...
		$order = $this->get_order(intval($id));
	...
	*/
	...
	/* GLOOBUS 03.12.2014 - Добавялем примечание к заказам из списка */
	public function add_note_order($ids = array(), $note) 
	{
		$query = $this->db->placehold("UPDATE __orders o SET o.note = case when LOWER(o.note) like LOWER('%".$this->db->escape(trim($note))."%') then o.note
															 when o.note = '' or LOWER(o.note) = LOWER(?) then ? else concat(o.note,'\n',?) end  
									   WHERE o.id in (?@)", $note, $note, $note, $ids);
