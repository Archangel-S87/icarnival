...
class Features extends Fivecms
{	
	...
	function get_features($filter = array())
	{
		...
		if(isset($filter['in_filter']))
			$in_filter_filter = $this->db->placehold('AND f.in_filter');
	...
	public function update_option2($product_id, $feature_id, $value)
	{	 

		$query = $this->db->placehold("SELECT * FROM __options 
										WHERE value=? AND  feature_id=? AND product_id=?   ", $value, intval($feature_id), intval($product_id));

		$this->db->query($query);
		if($row=$this->db->result()){
			// ToDo
		}else{
			$query = $this->db->placehold("REPLACE INTO __options SET value=?, product_id=?, feature_id=?", $value, intval($product_id), intval($feature_id));
			$this->db->query($query);
	...
	public function get_options($filter = array())
	{
		...
		$query = $this->db->placehold("SELECT po.product_id, po.feature_id, po.value, count(po.product_id) as count
		    ...
		    $visible_filter
			$category_id_filter
			WHERE 1 $feature_id_filter $product_id_filter $in_stock_filter $brand_id_filter $features_filter GROUP BY po.feature_id, po.value ORDER BY value=0, -value DESC, value");
	...
	{
		$query = $this->db->placehold("SELECT f.id as feature_id, f.name, f.in_filter, po.value, po.product_id FROM __options po LEFT JOIN __features f ON f.id=po.feature_id
										WHERE po.product_id in(?@) ORDER BY f.position, po.value", (array)$product_id);

		$this->db->query($query);
		return $this->db->results();
	}
	
	public function get_product_option_weight($product_id)
	{
		$res = 0;
		
		if(!empty($this->settings->feature_weight)){
			$feature_weight = intval($this->settings->feature_weight);
			$query = $this->db->placehold("SELECT po.value as value FROM __options po LEFT JOIN __features f ON f.id=po.feature_id WHERE po.product_id in(?@) AND f.id = $feature_weight ORDER BY f.position", (array)$product_id);
        ...
        if(empty($res) && !empty($this->settings->min_weight)){
        	$res = (float)$this->settings->min_weight;
        	$res = str_replace(',','.',$res);
        	$res = (float)$res;
        }
	...
	public function get_product_option_volume($product_id)
	{
		$res = 0;
		
		if(!empty($this->settings->feature_volume)){
			$feature_volume = intval($this->settings->feature_volume);
			$query = $this->db->placehold("SELECT po.value as value FROM __options po LEFT JOIN __features f ON f.id=po.feature_id WHERE po.product_id in(?@) AND f.id = $feature_volume ORDER BY f.position", (array)$product_id);
        ...
        if(empty($res) && !empty($this->settings->min_volume)){
        	$res = (float)$this->settings->min_volume;
        	$res = str_replace(',','.',$res);
        	$res = (float)$res;
        }	
		return round($res,3);
