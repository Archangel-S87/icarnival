...
class Image extends Fivecms
{
	private	$allowed_extentions = array('png', 'gif', 'jpg', 'jpeg', 'ico', 'webp', 'jp2', 'jxr');
	...
	function resize($filename, $type='')
	{
		if(empty($type)){
			...
			$original_images_dir = 'files/originals-'.$type.'/';
			$resized_images_dir = 'files/'.$type.'/';
		}
	...
	public function download_image($filename, $type='')
	{
		if(empty($type)){
			$original_images_dir = $this->config->original_images_dir;
			$resized_images_dir = $this->config->resized_images_dir;
		...
		while(file_exists($this->config->root_dir.$original_images_dir.$new_name))
		{
		...
		$local_file = $this->config->root_dir.$original_images_dir.$new_name;
	...
	public function upload_image($filename, $name, $type='')
	{
		if(empty($type)){
			$original_images_dir = $this->config->original_images_dir;
			$resized_images_dir = $this->config->resized_images_dir;
		...
		{			
			...
			if(move_uploaded_file($filename, $this->config->root_dir.$original_images_dir.$new_name))			
				return $new_name;
		}
	...
	{
		//$quality = 100;
		$quality = $this->settings->images_quality;
