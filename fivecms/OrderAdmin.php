...
class OrderAdmin extends Fivecms
{
	public function fetch()
	{
		...
		if(empty($total_volume)) $total_volume = 0;
		...
		if($this->request->get('view') == 'print') {
		    ...
		    // Сформировать товарную накладную
            try {
                $this->get_commodity_invoice($order, $purchases, $delivery);
            } catch (PHPExcel_Reader_Exception $e) {
            } catch (PHPExcel_Writer_Exception $e) {
            }
            return 0;
		} elseif($this->request->get('view') == 'invoice') {
		    // Сформировать счёт для юр. лиц
            try {
                $this->get_invoice($order, $purchases, $delivery);
            } catch (PHPExcel_Reader_Exception $e) {
            } catch (PHPExcel_Writer_Exception $e) {
    ...
    /**
     * Формирует товарный чек
     * @param $order
     * @param $purchases
     * @param $delivery
     * @param $subtotal
     * @param $payment_currency
     * @throws PHPExcel_Reader_Exception | null
     * @throws PHPExcel_Writer_Exception | null
     */
    private function get_commodity_check($order, $purchases, $delivery, $subtotal, $payment_currency)
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('A' . $row, $r + 2)
            ->setCellValue('C' . $row, !empty($delivery->name) ? $delivery->name : '')
     ...
     */
    private function get_commodity_invoice($order, $purchases, $delivery)
    {
        require_once 'classes/PHPExcel.php';
        $objReader = PHPExcel_IOFactory::createReader('Excel5');
        ...
        if ($order->delivery_price > 0 && $this->settings->torg12_delivery) {
            ...
            $dostavka->variant_id = '';
            $dostavka->product_name = !empty($delivery->name) ? $delivery->name : '';
     ...
     * @throws PHPExcel_Writer_Exception
     */
    private function get_invoice($order, $purchases, $delivery)
    {
