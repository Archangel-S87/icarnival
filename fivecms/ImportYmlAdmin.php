...
class ImportYmlAdmin extends Fivecms 
{
	...
	private $columns = array(
		...
		'brand'=>            'бренд',
		'variant'=>          'вариант',
		'variant1'=>         'размер (для одежды)',
		'variant2'=>         'цвет (для одежды)',
		'price'=>            'цена',
		'currency'=>         'currency',
		...
		'meta title'=>       'мета-заголовок страницы',
	...
	private $columns_compared = array(
		...
		//'param_Вариант' => 'variant',
		'typePrefix'=> 'typePrefix',
		'param_Размер' => 'variant1',
		'param_Цвет' => 'variant2',
	);
	...
	public function fetch() {
		...
		if($this->request->method('post') && $this->request->post("start_import_yml")){
			...
			if(isset($_POST['yml_import_currency'])){
				...
				//создаем валюту, если у нас такой еще нет 
	...
	private function convert_file($source, $dest) {
		...
		if(preg_match('//u', $teststring[1]) || (function_exists('mb_detect_encoding') && mb_detect_encoding($teststring[1], 'UTF-8'))){ // Кодировка - UTF8
			...
			while (($line = fgets($src, 4096)) !== false) {
				if($change_encoding == 0 && stripos($line, 'windows-1251') !== false){
					$line = str_ireplace('windows-1251', 'UTF-8', $line);
					$change_encoding = 1;
				}
		...
		while($xml->name === 'offer'){
			$node = new SimpleXMLElement($xml->readOuterXML());
				...
				foreach($node as $elem) {
					...
					//изменим имя тега url, чтобы не было совпадения с полем url 
					if($elem_name == 'url'){
						$elem_name = '#url#';
					}
