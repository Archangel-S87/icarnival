...
class RestProducts extends Rest
{
	...
	{
		parent::__construct();
		if(!$this->managers->access('products'))
		{
	...
	public function get()
	{
		...
		if($join = $this->request->get('join'))
		{
			...
			{				
				$properties = $this->features->get_product_options($items_ids);
				foreach($properties as $property) {
					$items[$property->product_id]->features[] = $property;
				}	
			}
		}
	...
	public function post()
	{
		...
		header("Location: ".$this->config->root_url."/fivecms/rest/products/".$id, true, 201);
	}
