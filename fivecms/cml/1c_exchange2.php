...
if($fivecms->request->get('type') == 'sale' && $fivecms->request->get('mode') == 'file')
{
	...
	foreach($xml->Документ as $xml_order)
	{
		...
		{
			$purchase = null;
			//  external_id товара и варианта (если есть) по 1С
			$product_1c_id = $variant_1c_id = '';
			@list($product_1c_id, $variant_1c_id) = explode('#', $xml_product->Ид);
			
			$variant_1c_id = $xml_product->Ид;
				
			// Ищем товар по внешнему ИД
			...
			// Ищем вариант товара по внешнему ИД, соответствующий товару с определенным id
...
if($fivecms->request->get('type') == 'sale' && $fivecms->request->get('mode') == 'query')
{
		...
		foreach($orders as $order)
		{
			...
			if(!empty($order->coupon_code) || !empty(round($order->bonus_discount))) {
				...
				if(!empty(round($order->bonus_discount))){
					$s1_1 = $s1->addChild ( "Скидка" );
...
function import_product($xml_product)
{
	...
	$product_sku = (string)$xml_product->Артикул;
	...
	if($fivecms->settings->onebrand == 1 && isset($xml_product->Изготовитель)){
		...
		// Найдем бренд по имени
		$fivecms->db->query('SELECT id FROM __brands WHERE name=?', $brand_name);
		$brand_id = $fivecms->db->result('id');
		if(empty($brand_id)) {
			// Создадим, если бренд не найден
			if($fivecms->settings->onemetatitle == 1){
	...
	// Единица измерения передана в БазовойЕдинице
	if($fivecms->settings->oneunits == 1 && !empty($xml_product->БазоваяЕдиница)) {
		$xml_unit = $xml_product->БазоваяЕдиница;
		
		$unit_external_id = strval($xml_product->БазоваяЕдиница['Код']);
		$unit = strval($xml_unit);
		if(!empty($unit_external_id) && !empty($unit)){
			$fivecms->db->query('SELECT unit FROM __onec_units WHERE external_id=?', $unit_external_id);
			$unit_name = $fivecms->db->result('unit');
			// Если данной единицы не существует, то добавляем
	...
	// Если нужно - удаляем товар
	if(!empty($xml_product->ПометкаУдаления) && $xml_product->ПометкаУдаления == 'true' && !empty($product_id)){
...
function import_variant($xml_variant)
{
	...
	@list($product_1c_id, $variant_1c_id_cut) = explode('#', $xml_variant->Ид);
	
	// Пишем полный ИД с #
	$variant_1c_id = $xml_variant->Ид;
	
	if(empty($product_1c_id))
		return false;

	// Проверяем если вариант с таким ИД уже создан и привязан к товару с таким ИД
	...
	// Получаем id товара с таким ИД и добавляем его к варианту
	...
	// Формируем название варианта из ХарактеристикиТовара
	if(!empty($values))
		$variant->name = implode(', ', $values);
		
	// Пишем название и цвет	
	if($fivecms->settings->onesizecol == 1){
		if(!empty($size))
			$variant->name1 = $size;
		if(!empty($color))
	...
	// Создаем или обновляем вариант товара
	if(empty($variant_id)) {
