{capture name=tabs}
	...
	{if in_array('blog', $manager->permissions)}<li><a href="index.php?module=BlogAdmin">{$tr->blog|escape}</a></li>{/if}
	{if in_array('articles_categories', $manager->permissions)}<li><a href="index.php?module=ArticlesCategoriesAdmin">{$tr->articles_categories|escape}</a></li>{/if}
	<li class="active"><a href="index.php?module=ArticlesAdmin{if !empty($post->category_id)}&category_id={$post->category_id}{/if}">{$tr->articles|escape}</a></li>
{/capture}
...
$(function() {
	...
	// Auto fill metadata | Автозаполнение мета-тегов
...
<div id="onecolumn">
	...
	<div class="message message_success">
		...
		<a class="link" target="_blank" href="../article/{$post->url}">{$tr->open_on_page|escape}</a>
		{if isset($smarty.get.return)}
			<a class="button" href="{$smarty.get.return}">{$tr->return|escape}</a>
		{/if}
	
		<span class="share">		
			<a href="#" onClick='window.open("http://vkontakte.ru/share.php?url={$config->root_url|urlencode}/article/{$post->url|urlencode}&title={$post->name|urlencode}&description={$post->annotation|urlencode}&noparse=false","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
	  		<img src="{$config->root_url}/fivecms/design/images/vk_icon.png" /></a>
			<a href="#" onClick='window.open("http://www.facebook.com/sharer.php?u={$config->root_url|urlencode}/article/{$post->url|urlencode}","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
	  		<img src="{$config->root_url}/fivecms/design/images/facebook_icon.png" /></a>
			<a href="#" onClick='window.open("http://twitter.com/share?text={$post->name|urlencode}&url={$config->root_url|urlencode}/article/{$post->url|urlencode}&hashtags={$post->meta_keywords|replace:' ':''|urlencode}","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
	...
	<input type=hidden name="session_id" value="{$smarty.session.id}">
		...
		<div id="product_categories" {*{if !$articles_categories}style='display:none;'{/if}*}>
		...
		<div id="column_left">
			...
			<!-- Parameters of page | Параметры страницы -->
			<div class="block">
				<ul>
					<li><label style="width:90px;" class=property>{$tr->date|escape}</label><input style="width:70px;" type=text name=date value='{$post->date|date}'></li>
				</ul>
			</div>
			<div class="block layer">
				<h2>{$tr->page_parameters|escape}</h2>
				<ul>
					<li style="width: 100%;"><label style="width: 90px;" class=property>{$tr->url|escape}</label><div class="page_url" style="width: 35px;"> /article/</div><input style="width: 414px;" name="url" class="page_url" type="text" value="{if !empty($post->url)}{$post->url|escape}{/if}" /></li>
					...
					<li style="width: 100%;"><label style="width: 90px;" class=property>{$tr->keywords|escape}</label><input maxlength="250" style="width: 450px;" id="text-count2" name="meta_keywords"  type="text" value="{if !empty($post->meta_keywords)}{$post->meta_keywords|escape}{/if}" /><span style="margin-left: 5px;" id="count2"></span></li>
			...
			<!-- Parameters of page | Параметры страницы (The End)-->
	...
	{/literal}
		...
		<div id="column_right">
			...
			<div class="block layer images">
				...
				<ul>
					{assign var=random value=10|mt_rand:1000}
					{$i=0}
