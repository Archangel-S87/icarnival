{capture name=tabs}
	...
	<li class="active"><a href="index.php?module=TemplatesAdmin">{$tr->templates|escape}</a></li>		
...
{if $template_file}
{$meta_title = "$template_file" scope=root}
{/if}
...
<h1>{$tr->theme}: {$theme}, {$tr->template|lower}: {$template_file}</h1>

{if isset($message_error)}
<!-- Системное сообщение -->
<div class="message message_error">
	<span class="text">
	{if $message_error == 'permissions'}{$tr->set_permissions} {$template_file}
...
<div class="block layer">
	<div class="templates_names">
		{foreach item=t from=$templates}
			{if $t=='mp.tpl'}{else}<a {if $template_file == $t}class="selected"{/if} href='index.php?module=TemplatesAdmin&file={$t}'>{$t}</a>{/if}
		{/foreach}
	</div>
</div>
