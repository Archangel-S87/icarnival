...
{if $style_file}
{$meta_title = "$style_file ({$tr->mob|escape})" scope=root}
{/if}
...
<h1>{$tr->theme|escape}: {$theme}, CSS: {$style_file}</h1>

{if isset($message_error)}
<!-- Системное сообщение -->
<div class="message message_error">
	<span class="text">
	{if $message_error == 'permissions'}{$tr->set_permissions|escape} {$style_file}
...
<div class="block layer">
	<div class="templates_names">
		{foreach item=s from=$styles}
			<a {if $style_file == $s}class="selected"{/if} href='index.php?module=MobileStylesAdmin&file={$s}'>{$s}</a>
		{/foreach}
	</div>
</div>
